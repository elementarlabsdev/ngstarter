...
export function getPointFromEvent(event: MouseEvent | TouchEvent): SplitPoint | null {
  // TouchEvent
  if (
...
export function getAreaMinSize(a: SplitArea): number {
  if (a.size === null) {
    return 0;
  }
  ...
  if ((a.component.getMinSize() as number) > a.size) {
    return a.size;
  }

  return a.component.getMinSize() as number;
}

export function getAreaMaxSize(a: SplitArea): null | number {
  if (a.size === null) {
    return null;
  }
  ...
  if ((a.component.getMaxSize() as number) < a.size) {
    return a.size;
  }

  return a.component.getMaxSize() as number;
}

export function getGutterSideAbsorptionCapacity(
  unit: 'percent' | 'pixel',
  sideAreas: SplitAreaSnapshot[],
  pixels: number,
...
export function getAreaAbsorptionCapacity(
  ...
  pixels: number,
  allAreasSizePixel: number
): SplitAreaAbsorptionCapacity | void {
  // No pain no gain
  if (pixels === 0) {
    return {
      areaSnapshot,
...
export function updateAreaSize(unit: 'percent' | 'pixel', item: SplitAreaAbsorptionCapacity) {
  if (unit === 'percent') {
    item.areaSnapshot.area.component.setSize(item.percentAfterAbsorption);
  } else if (unit === 'pixel') {
    // Update size except for the wildcard size area
    if (item.areaSnapshot.area.size !== null) {
      item.areaSnapshot.area.component.setSize(
